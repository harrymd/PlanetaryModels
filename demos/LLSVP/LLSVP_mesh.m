...
%% Create or load triangulations on spherical interfaces.

tic;

% Estimate the number of triangles on the free surface.
% First option: Based on ratio of surface areas between the sphere and
% a tetrahedral element. (Seems to give too few tetrahedra, perhaps
% multiplying by 30 would give better results.)
% a_surf          = 4.0*pi*(R1^2.0);
% a_face_tet_max  = 8.0*(3.0^(-7.0/6.0))*(tet_max_vol^(2.0/3.0));
% n_tri_surf      = round(a_surf/a_face_tet_max);
...
%     % Estimate the number of triangles on the core-mantle and inner-core
%     % boundaries. In this case, we use the same number on the CMB as at the free
%     % surface (as we are interested in the CMB) but fewer on the ICB (as it is
%     % a smaller interface).
%     n_tri_surf_over_n_tri_cmb = 1.0;
%     n_tri_surf_over_n_tri_icb = 2.0;
%     n_tri_cmb = round(n_tri_surf/n_tri_surf_over_n_tri_cmb);
%     n_tri_icb = round(n_tri_surf/n_tri_surf_over_n_tri_icb);
...
% Calculate unit spheres (if they do not already exist)
%dir_unit_sphere         = '../../unitspheres/data/';
dir_unit_sphere = '../../output/unit_spheres/';
unit_sphere_file_surf   = unit_sphere(d_surf,   dir_unit_sphere, distmesh);
unit_sphere_file_cmb    = unit_sphere(d_cmb,    dir_unit_sphere, distmesh);
unit_sphere_file_icb    = unit_sphere(d_icb,    dir_unit_sphere, distmesh);
  
% Load unit sphere and then scale by radius.
% The radius must be slightly adjusted to give a constant volume.
...
load(unit_sphere_file_surf)
vol_frac_1      = vol/((4.0/3.0)*pi);
R1_adjusted     = ((1.0/vol_frac_1)^(1.0/3.0))*R1;
p1              = R1_adjusted*p;         % Points on surface of Earth.
np1             = size(p1,1);
t1              = t;
nt1             = size(t1,1);

% Core-mantle boundary.
load(unit_sphere_file_cmb)
vol_frac_2      = vol/((4.0/3.0)*pi);
R2_adjusted     = ((1.0/vol_frac_2)^(1.0/3.0))*R2;
p2              = R2_adjusted*p;         % Points on surface of Earth.
np2             = size(p2,1);
t2              = t;          
nt2             = size(t2,1);

% Surface of the inner core.
load(unit_sphere_file_icb)
vol_frac_3      = vol/((4.0/3.0)*pi);
R3_adjusted     = ((1.0/vol_frac_3)^(1.0/3.0))*R3;
p3              = R3_adjusted*p;         % Points on surface of Earth.
np3             = size(p3,1);
t3              = t;
nt3             = size(t3,1);
...
mod_name = 'mod';
dir_name = sprintf('%06.1f_%4.2f_%1d',   ...
...
% Save the adjusted discontinuity radii. 
file_r_adjusted = 'r_adjusted.txt';
path_r_adjusted = fullfile(dir_out_llsvp, file_r_adjusted);
f_id_r_adjusted = fopen(path_r_adjusted, 'w');
fprintf(f_id_r_adjusted, '%18.12e\n', R1_adjusted);
fprintf(f_id_r_adjusted, '%18.12e\n', R2_adjusted);
fprintf(f_id_r_adjusted, '%18.12e\n', R3_adjusted);
fclose(f_id_r_adjusted);
...
% Copy adjusted radii file.
path_r_adjusted_ref = fullfile(dir_out_ref, file_r_adjusted);
copyfile(path_r_adjusted, path_r_adjusted_ref);
